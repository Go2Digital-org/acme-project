<?php

declare(strict_types=1);

namespace Modules\Organization\Domain\Event;

use DateTimeInterface;
use Modules\Shared\Application\Event\AbstractDomainEvent;

final class OrganizationDeactivatedEvent extends AbstractDomainEvent
{
    public function __construct(
        public readonly int $organizationId,
        public readonly string $reason,
        public readonly DateTimeInterface $deactivatedAt,
    ) {
        parent::__construct($this->organizationId);
    }

    public function getEventName(): string
    {
        return 'organization.deactivated';
    }

    public function getEventData(): array
    {
        return array_merge(parent::getEventData(), [
            'organization_id' => $this->organizationId,
            'reason' => $this->reason,
            'deactivated_at' => $this->deactivatedAt->format('Y-m-d H:i:s'),
        ]);
    }

    public function isAsync(): bool
    {
        return true; // Async for background processing
    }
}
