...
final class LargeDonationNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail(mixed $notifiable): MailMessage
    {
        ...
        return (new MailMessage)
            ->subject('Large Donation Alert - ' . $amount)
            ->greeting('Large Donation Received!')
            ->line("{$donorName} has made a significant donation of {$amount}.")
            ...
            ->line('Payment Method: ' . ($this->donation->payment_method?->getLabel() ?? 'Unknown'))
            ->action('View Donation', route('filament.admin.resources.donations.view', $this->donation))
            ->line('This donation requires your attention due to its size.')
    ...
    public function toDatabase(mixed $notifiable): DatabaseMessage
    {
        ...
        return new DatabaseMessage([
            ...
            'message' => $donorName . ' donated ' . $amount . ' to ' . ($this->donation->campaign->title ?? 'Unknown Campaign'),
