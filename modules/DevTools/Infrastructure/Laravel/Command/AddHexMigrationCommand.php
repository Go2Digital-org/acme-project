...
class AddHexMigrationCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'hex:add:migration {domain? : Domain name} {name? : Migration name} {--force : Force overwrite existing files}';

    /**
     * @var string
     */
    protected $description = 'Add a migration to a specific domain';
    ...
    public function handle(): int
    {
        ...
        $tableName = $this->ask('Enter the table name (leave blank to use domain name)', Str::lower($domain ?? ''));
        ...
        $this->createMigration($basePath, $domain, $tableName !== '' ? $tableName : Str::lower($domain));
        $this->info("Migration has been added to the {$domain} domain.");
    ...
    private function createMigration(string $basePath, string $domain, string $tableName): void
    {
        $timestamp = date('Y_m_d_His');
        $migrationName = "create_{$tableName}_table";
        $filePath = "{$basePath}/Infrastructure/Laravel/Migration/{$timestamp}_{$migrationName}.php";
        ...
        $this->createFileFromStub($filePath, $stubPath, [
            ...
            'tableNameStudly' => Str::studly($tableName),
        ]);
    }
